...
class Dealer:
    ...
    def search(self, req, idx_names: str | list[str],
        ...
        if not qst:
            ...
            #     logging.debug("Dealer.search TOTAL: {}".format(total))
            # else:
            # 不管有没有 emb_mdl，都强制做向量检索， 生成向量检索表达式 matchDense
        ...
        return self.SearchResult(
            total=total, # 本次检索命中的总条数
            ids=ids, # 命中的 chunk ID 列表
            query_vector=q_vec, # 用户问题经过 embedding 后得到的查询向量
            aggregation=aggs, # 聚合统计结果
            highlight=highlight, # 高亮结果
            field=self.dataStore.getFields(res, src), # 每个命中 chunk 的详细字段内容
            keywords=keywords # 从用户问题中解析出的关键词列表
        )
    ...
    def retrieval(self, question, embd_mdl, tenant_ids, kb_ids, page, page_size, similarity_threshold=0.2,
        ...
        """
        question                  用户问题
        embd_mdl                  embedding 模型，用来生成 query 向量
        tenant_ids                租户 ID，决定查哪个索引
        kb_ids                    知识库 ID，决定查哪个知识库
        page                      当前页码
        page_size                 每页返回多少条
        similarity_threshold      相似度阈值，低于这个分数就不要
        ...
        rerank_mdl                重排模型
        highlight                 是否返回高亮文本
        rank_feature              排序增强特征，比如 PageRank 权重
        """
        ...
        #利用稠密检索和稀疏检索在数据库里面搜索相似内容,返回SearchResult 对象
        sres = self.search(req, [index_name(tid) for tid in tenant_ids],
                           kb_ids, embd_mdl, highlight, rank_feature=rank_feature)
        
        '''
        sres.total         命中总数
        sres.ids           命中的 chunk id 列表
        sres.query_vector  用户问题的向量
        sres.field         每个 chunk id 对应的字段内容
        sres.highlight     高亮信息
        '''
        ...
        if page <= RERANK_PAGE_LIMIT:
            if sres.total > 0:
                ...
                '''
                sim    最终综合相似度
                tsim   term similarity，文本/关键词相似度
                vsim   vector similarity，向量相似度
                '''
